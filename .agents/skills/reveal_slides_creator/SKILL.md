...
- **Único Retorno (Single Return):** Evitar múltiples retornos (`return`) en una misma función y, en particular, **no utilizar return dentro de bucles for**. Si se necesita detener la ejecución de un bucle de búsqueda de forma anticipada, se debe utilizar un bucle `while` con una condición compuesta y una bandera lógica (ej. `encontrado`), retornando una única variable de resultado al final de la función.
